...
> ⚠️ Если сервис слушает HTTP-порт — в `docker-compose.yml` привязывай его к `127.0.0.1`, иначе Docker обойдёт UFW и порт будет открыт в интернет:
> ```yaml
> ports:
>   - "127.0.0.1:8000:8000"   # правильно
>   # - "8000:8000"            # небезопасно
> ```
